using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class SoundPlay : MonoBehaviour
{
    public enum SoundsType
    {
        // SE
        None,
        HitL,
        HitM,
        HitS,
        GuardL,
        GuardM,
        GuardS,
        JabL,
        JabM,
        JabS,
        Squat,
        Jump,
        Step,
        Down,
        DownCancel,
        GetUp,
        Sutan,
        Special1,
        Special1_Hit,
        Ca_Hit,
		Menu_MoveCursor,
		Menu_Cancel,
		Menu_Decision,

		// CV_Oba
		Oba_CharacterSelect,
        Oba_GetUp_DownCancel,
        Oba_HitL,
        Oba_HitM,
        Oba_HitS_Down,
        Oba_JobL,
        Oba_JobM_Throw,
        Oba_JobS,
        Oba_Jump_Step,
        Oba_BattleDraw,
        Oba_RoundLoss,
        Oba_BattleWin,
        Oba_Special1,
        Oba_Special2,
        Oba_Special3,
        Oba_Special4,
        Oba_Thrown,
        Oba_RoundStart,

        // CV_Culico
        Clico_CharacterSelect,
        Clico_GetUp_DownCancel,
        Clico_HitL,
        Clico_HitM,
        Clico_HitS_Down,
        Clico_JobL,
        Clico_JobM_Throw,
        Clico_JobS,
        Clico_Jump_Step,
        Clico_BattleDraw,
        Clico_RoundLoss,
        Clico_BattleWin,
        Clico_Special1,
        Clico_Special2,
        Clico_Special3,
        Clico_Special4,
        Clico_Thrown,
        Clico_RoundStart,

		// CV_Kuidaore
		Kuidaore_CharacterSelect,
		Kuidaore_GetUp_DownCancel,
		Kuidaore_HitL,
		Kuidaore_HitM,
		Kuidaore_HitS_Down,
		Kuidaore_JobL,
		Kuidaore_JobM_Throw,
		Kuidaore_JobS,
		Kuidaore_Jump_Step,
		Kuidaore_BattleDraw,
		Kuidaore_RoundLoss,
		Kuidaore_BattleWin,
		Kuidaore_Special1,
		Kuidaore_Special2,
		Kuidaore_Special3,
		Kuidaore_Special4,
		Kuidaore_Thrown,
		Kuidaore_RoundStart,

		// EVA
		EV_ATField,
		EV_NExtStage,
		EV_EVA_Attack1,
		EV_EVA_Attack2,
		EV_EVA_Attack3,
		EV_EVA_Attack4,
		EV_EVA_Command1,
		EV_EVA_Command2,
		EV_EVA_Deathblow,
		EV_EVA_Guard,
		EV_EVA_Start,
		EV_EVA_Victory,
		EV_SHITO_Attack1,
		EV_SHITO_Attack2,
		EV_SHITO_Attack3,
		EV_SHITO_Command1,
		EV_SHITO_Command2,
		EV_SHITO_Command3,
		EV_SHITO_Deathblow,
		EV_SHITO_Start,
		EV_SHITO_Victory,
		EV_Shinji_RoundStart,

		Oba_Hit,
		Clico_Tackle,
        Beams,
        Nice,
        Great,
        Excellent,

		CharaVoice_Clico,
		CharaVoice_Oba,
		CharaVoice_Kuidaore

	}
    public SoundsType soundsType;

    int i = 0;
    int timeCount = 0;
    public bool destroyFlag = true;
    public int frame = 30;

	public float seVolume = 0.25f;			//SEのボリューム
	public float cvVolume = 0.25f;			//キャラクターボイスのボリューム
	private float KuidaorePichSpeed=1f;	//食い倒れボイスのピッチ

	void Start()
    {
        Sound.AllSoundLod();
}

    void Update()
    {
        if (i == 0)
        {
            switch (soundsType)
            {
                case SoundsType.None:
                    break;
                case SoundsType.HitL:
                    Sound.PlaySE("HitL", 1, seVolume, 1);
                    break;
                case SoundsType.HitM:
                    Sound.PlaySE("HitS", 1, seVolume, 1);
                    break;
                case SoundsType.HitS:
                    Sound.PlaySE("HitS", 1, seVolume, 1);
                    break;
                case SoundsType.GuardL:
                    Sound.PlaySE("GuardL", 2, seVolume, 1);
                    break;
                case SoundsType.GuardM:
                    Sound.PlaySE("GuardM", 2, seVolume, 1);
                    break;
                case SoundsType.GuardS:
                    Sound.PlaySE("GuardS", 2, seVolume, 1);
                    break;
                case SoundsType.JabL:
                    Sound.PlaySE("JabL", 3, seVolume, 1);
                    break;
                case SoundsType.JabM:
                    Sound.PlaySE("JabM", 3, seVolume, 1);
                    break;
                case SoundsType.JabS:
                    Sound.PlaySE("JabS", 3, seVolume, 1);
                    break;
                case SoundsType.Squat:
                    Sound.PlaySE("Squat", 4, seVolume, 1);
                    break;
                case SoundsType.Jump:
                    Sound.PlaySE("Jump", 4, seVolume, 1);
                    break;
                case SoundsType.Step:
                    Sound.PlaySE("Step", 5, seVolume, 1);
                    break;
                case SoundsType.Down:
                    Sound.PlaySE("Down", 6, seVolume, 1);
                    break;
                case SoundsType.DownCancel:
                    Sound.PlaySE("DownCancel", 7, seVolume, 1);
                    break;
                case SoundsType.GetUp:
                    Sound.PlaySE("GetUp", 8, seVolume, 1);
                    break;
                case SoundsType.Sutan:
                    Sound.PlaySE("Sutan", 1, seVolume, 1);
                    break;
                case SoundsType.Special1:
                    Sound.PlaySE("Special1", 9, seVolume, 1);
                    break;
                case SoundsType.Special1_Hit:
                    Sound.PlaySE("Special1_Hit", 10, seVolume, 1);
                    break;
                case SoundsType.Ca_Hit:
                    Sound.PlaySE("Ca_Hit", 11, seVolume, 1);
                    break;
				
				//おばちゃん
				case SoundsType.Oba_CharacterSelect:
                    Sound.PlaySE("Oba_CharacterSelect", 1, cvVolume, 1);
                    break;
                case SoundsType.Oba_GetUp_DownCancel:
                    Sound.PlaySE("Oba_GetUp_DownCancel", 12, cvVolume, 1);
                    break;
                case SoundsType.Oba_HitL:
                    Sound.PlaySE("Oba_HitL", 13, cvVolume, 1);
                    break;
                case SoundsType.Oba_HitM:
                    Sound.PlaySE("Oba_HitM", 13, cvVolume, 1);
					break;
                case SoundsType.Oba_HitS_Down:
                    Sound.PlaySE("Oba_HitS_Down", 13, cvVolume, 1);
                    break;
				case SoundsType.Oba_Hit:
					Sound.PlaySE("Oba_HitS_Down", 13, cvVolume, 1);
					break;
				case SoundsType.Oba_JobL:
					Sound.PlaySE("Oba_JobL", 14, cvVolume, 1);
					break;
				case SoundsType.Oba_JobM_Throw:
					Sound.PlaySE("Oba_JobM_Throw", 14, cvVolume, 1);
					break;
				case SoundsType.Oba_JobS:
					Sound.PlaySE("Oba_JobS", 14, cvVolume, 1);
					break;
				case SoundsType.Oba_Jump_Step:
                    Sound.PlaySE("Oba_Jump_Step", 15, cvVolume, 1);
                    break;
                case SoundsType.Oba_BattleDraw:
                    Sound.PlaySE("Oba_BattleDraw", 16, cvVolume, 1);
                    break;
                case SoundsType.Oba_RoundLoss:
                    Sound.PlaySE("Oba_RoundLoss", 16, cvVolume, 1);
                    break;
                case SoundsType.Oba_BattleWin:
                    Sound.PlaySE("Oba_BattleWin", 16, cvVolume, 1);
                    break;
                case SoundsType.Oba_Special1:
                    Sound.PlaySE("Oba_Special1", 17, cvVolume, 1);
                    break;
                case SoundsType.Oba_Special2:
                    Sound.PlaySE("Oba_Special2", 14, cvVolume, 1);
                    break;
                case SoundsType.Oba_Special3:
                    Sound.PlaySE("Oba_Special3", 17, cvVolume, 1);
                    break;
                case SoundsType.Oba_Special4:
                    Sound.PlaySE("Oba_Special4", 17, cvVolume, 1);
                    break;
                case SoundsType.Oba_Thrown:
                    Sound.PlaySE("Oba_Thrown", 18, cvVolume, 1);
                    break;
                case SoundsType.Oba_RoundStart:
						Sound.PlaySE("Oba_RoundStart", 19, cvVolume, 1);
                    break;

				//クリコ
                case SoundsType.Clico_CharacterSelect:
                    Sound.PlaySE("Clico_CharacterSelect", 1, cvVolume, 1);
                    break;
                case SoundsType.Clico_GetUp_DownCancel:
                    Sound.PlaySE("Clico_GetUp_DownCancel", 12, cvVolume, 1);
                    break;
                case SoundsType.Clico_HitL:
                    Sound.PlaySE("Clico_HitL", 13, cvVolume, 1);
                    break;
                case SoundsType.Clico_HitM:
                    Sound.PlaySE("Clico_HitM", 13, cvVolume, 1);
                    break;
                case SoundsType.Clico_HitS_Down:
                    Sound.PlaySE("Clico_HitS_Down", 13, cvVolume, 1);
                    break;
                case SoundsType.Clico_JobL:
                    Sound.PlaySE("Clico_JobL", 14, cvVolume, 1);
                    break;
                case SoundsType.Clico_JobM_Throw:
                    Sound.PlaySE("Clico_JobM_Throw", 14, cvVolume, 1);
                    break;
                case SoundsType.Clico_JobS:
                    Sound.PlaySE("Clico_JobS", 14, cvVolume, 1);
                    break;
                case SoundsType.Clico_Jump_Step:
                    Sound.PlaySE("Clico_Jump_Step", 15, cvVolume, 1);
                    break;
                case SoundsType.Clico_BattleDraw:
                    Sound.PlaySE("Clico_BattleDraw", 16, cvVolume, 1);
                    break;
                case SoundsType.Clico_RoundLoss:
                    Sound.PlaySE("Clico_RoundLoss", 16, cvVolume, 1);
                    break;
                case SoundsType.Clico_BattleWin:
                    Sound.PlaySE("Clico_BattleWin", 16, cvVolume, 1);
					break;
                case SoundsType.Clico_Special1:
                    Sound.PlaySE("Clico_Special1", 17, cvVolume, 1);
                    break;
                case SoundsType.Clico_Special2:
                    Sound.PlaySE("Clico_Special2", 17, cvVolume, 1);
                    break;
                case SoundsType.Clico_Special3:
                    Sound.PlaySE("Clico_Special3", 17, cvVolume, 1);
                    break;
                case SoundsType.Clico_Special4:
                    Sound.PlaySE("Clico_Special4", 17, cvVolume, 1);
                    break;
                case SoundsType.Clico_Thrown:
                    Sound.PlaySE("Clico_Thrown", 18, cvVolume, 1);
                    break;
                case SoundsType.Clico_RoundStart:
					Sound.PlaySE("Clico_RoundStart", 18, cvVolume, 1);
					break;
				case SoundsType.Clico_Tackle:
					Sound.PlaySE("Clico_Tackle", 19, cvVolume, 1);
					break;

				//食い倒れ
				case SoundsType.Kuidaore_CharacterSelect:
					Sound.PlaySE("Kuidaore_CharacterSelect", 1, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_GetUp_DownCancel:
					Sound.PlaySE("Kuidaore_GetUp_DownCancel", 12, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_HitL:
					Sound.PlaySE("Kuidaore_HitL", 13, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_HitM:
					Sound.PlaySE("Kuidaore_HitM", 13, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_HitS_Down:
					Sound.PlaySE("Kuidaore_HitS_Down", 13, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_JobL:
					Sound.PlaySE("Kuidaore_JobL", 14, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_JobM_Throw:
					Sound.PlaySE("Kuidaore_JobM_Throw", 14, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_JobS:
					Sound.PlaySE("Kuidaore_JobS", 14, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_Jump_Step:
					Sound.PlaySE("Kuidaore_Jump_Step", 15, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_BattleDraw:
					Sound.PlaySE("Kuidaore_BattleDraw", 16, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_RoundLoss:
					Sound.PlaySE("Kuidaore_RoundLoss", 16, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_BattleWin:
					Sound.PlaySE("Kuidaore_BattleWin", 16, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_Special1:
					Sound.PlaySE("Kuidaore_Special1", 17, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_Special2:
					Sound.PlaySE("Kuidaore_Special2", 14, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_Special3:
					Sound.PlaySE("Kuidaore_Special3", 17, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_Special4:
					Sound.PlaySE("Kuidaore_Special4", 17, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_Thrown:
					Sound.PlaySE("Kuidaore_Thrown", 18, cvVolume, KuidaorePichSpeed);
					break;
				case SoundsType.Kuidaore_RoundStart:
					Sound.PlaySE("Kuidaore_RoundStart", 18, cvVolume, KuidaorePichSpeed);
					break;

				//Evangerion
				case SoundsType.EV_ATField:
					Sound.PlaySE("EV_ATField", 2, 1, 1);
					break;
				case SoundsType.EV_EVA_Attack1:
					Sound.PlaySE("EV_EVA_Attack1", 14, 1, 1);
					break;
				case SoundsType.EV_EVA_Attack2:
					Sound.PlaySE("EV_EVA_Attack2", 14, 1, 1);
					break;
				case SoundsType.EV_EVA_Attack3:
					Sound.PlaySE("EV_EVA_Attack3", 14, 1, 1);
					break;
				case SoundsType.EV_EVA_Attack4:
					Sound.PlaySE("EV_EVA_Attack4", 14, 1, 1);
					break;
				case SoundsType.EV_EVA_Deathblow:
					Sound.PlaySE("EV_EVA_Deathblow", 16, 1, 1);
					break;
				case SoundsType.EV_EVA_Victory:
					Sound.PlaySE("EV_EVA_Victory", 16, 1, 1);
					break;
				case SoundsType.EV_EVA_Command1:
					Sound.PlaySE("EV_EVA_Command1", 17, 1, 1);
					break;
				case SoundsType.EV_EVA_Command2:
					Sound.PlaySE("EV_EVA_Command2", 17, 1, 1);
					break;
				case SoundsType.EV_EVA_Start:
					Sound.PlaySE("EV_EVA_Start", 19, 1, 1);
					break;
				case SoundsType.EV_SHITO_Attack1:
					Sound.PlaySE("EV_SHITO_Attack1", 14, 0.6f, 1);
					break;
				case SoundsType.EV_SHITO_Attack2:
					Sound.PlaySE("EV_SHITO_Attack2", 14, 0.6f, 1);
					break;
				case SoundsType.EV_SHITO_Attack3:
					Sound.PlaySE("EV_SHITO_Attack3", 14, 0.6f, 1);
					break;
				case SoundsType.EV_SHITO_Deathblow:
					Sound.PlaySE("EV_SHITO_Deathblow", 16, 0.6f, 1);
					break;
				case SoundsType.EV_SHITO_Victory:
					Sound.PlaySE("EV_SHITO_Victory", 16, 0.6f, 1);
					break;
				case SoundsType.EV_SHITO_Command1:
					Sound.PlaySE("EV_SHITO_Command1", 17, 0.6f, 1);
					break;
				case SoundsType.EV_SHITO_Command2:
					Sound.PlaySE("EV_SHITO_Command2", 17, 0.6f, 1);
					break;
				case SoundsType.EV_SHITO_Command3:
					Sound.PlaySE("EV_SHITO_Command3", 17, 0.6f, 1);
					break;
				case SoundsType.EV_SHITO_Start:
					Sound.PlaySE("EV_SHITO_Start", 19, 0.6f, 1);
					break;
				case SoundsType.EV_Shinji_RoundStart:
					Sound.PlaySE("EV_Shinji_RoundStart", 18, 3, 1);
					break;

				case SoundsType.CharaVoice_Clico:
					Sound.PlaySE("CharaVoice_Clico", 1, 0.6f, 1);
					break;
				case SoundsType.CharaVoice_Oba:
					Sound.PlaySE("CharaVoice_Oba", 1, 0.6f, 1);
					break;
				case SoundsType.CharaVoice_Kuidaore:
					Sound.PlaySE("CharaVoice_Kuidaore", 1, 0.6f, 1);
					break;

				case SoundsType.Beams:
                    Sound.PlaySE("SE_Beams", 18, 0.5f, 1);
                    break;
                case SoundsType.Nice:
                    Sound.PlaySE("SV_Nice", 19, 1, 1);
                    break;
                case SoundsType.Great:
                    Sound.PlaySE("SV_Great", 19, 0.5f, 1);
                    break;
                case SoundsType.Excellent:
                    Sound.PlaySE("SV_Excellent", 19, 0.5f, 1);
					break;
            }
            i++;
        }

        if (destroyFlag)
        {
            DestroyObject(frame);
        }
    }

    void DestroyObject(int frame)
    {
        timeCount += 1;
        if (timeCount == frame)
        {
            Destroy(gameObject);
        }
    }
}

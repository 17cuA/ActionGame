...
Shader "Unlit/Toon_ShadowColor"
{
	...
	_1st_ShadowColor("1st_ShadowColor",Color) = (0,0,0,0)
	_2st_ShadowColor("2st_ShadowColor",Color) = (0,0,0,0)
		...
		SubShader
	{
		...
		float3 lerp3(float3 x, float3 y, float s)
		{
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR
			{
				...
				float3 _1st_ShadeColor = _1st_ShadowColor.rbg  * _Set_LightColor;
				// 2影
				//float4 _2st_ShadeMap_var = tex2D(_2st_ShadeMap, TRANSFORM_TEX(i.uv0, _2st_ShadeMap));
				float3 _2st_ShadeColor = _2st_ShadowColor.rgb * _Set_LightColor;
		...
		float4 frag(VertexOutput i, float facting : VFACE) : SV_Target
		{
			...
			float3 Set__ModelOutLine_Color = lerp(
				__ModelOutLine_Color.rgb,
				__ModelOutLine_Color.rgb * Set_BaseColor * Set_BaseColor,
				_Is_BlendBaseColor);

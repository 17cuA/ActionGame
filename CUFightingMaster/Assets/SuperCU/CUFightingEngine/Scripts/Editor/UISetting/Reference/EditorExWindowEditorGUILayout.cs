...
public class EditorExWindowEditorGUILayout : ScriptableWizard
{
	[MenuItem("見本/EditorGUILayout")]
	static void Open()
	{
		EditorWindow.GetWindow<EditorExWindowEditorGUILayout>("EditorGUILayout");
	}
	...
	bool inspectorTitlebarGameObj = false;
	...
	void OnGUI()
	{
		...
		gameObject = EditorGUILayout.ObjectField("ObjectField", gameObject, typeof(GameObject), true) as GameObject;

		if (gameObject != null)
		{
			inspectorTitlebarGameObj = EditorGUILayout.InspectorTitlebar(inspectorTitlebarGameObj, gameObject);
			if (inspectorTitlebarGameObj)
			{
				EditorGUILayout.LabelField("ﾁﾗｯﾁﾗｯ");
			}
		}

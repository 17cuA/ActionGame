	...
	private void InputCommandApplyCheck()
	{
		...
		if (!isCommandInterval)
		{
            ...
            for (int i = 0; attackParameters.Count > i; i++)
			{
                ...
                //コマンドを発動する処理
                if (attackParameters[i].isShot == true && isShotCommand == false)
                {
                    //コマンド攻撃処理
                    if (testInput.atkButton != "" && Regex.IsMatch(attackParameters[i].shotTrigger, testInput.atkButton))
                    {
                        //コマンドを保存
